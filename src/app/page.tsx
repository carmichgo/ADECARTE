...
export default function Home() {
  ...
  const accountContextForAI = (() => {
    ...
    return `\n## OUR ACCOUNTS (owned by us)\n${lines}\n\nLOC-RELATED ACCOUNTS: ${locList}\n\nIMPORTANT RULES FOR CATEGORIZATION:\n\n1. INTERNAL TRANSFERS: A transfer is "Internal Transfer" ONLY if BOTH source AND destination are our accounts.\n\n2. LINE OF CREDIT RULES:\n   THE ENTIRE LINE OF CREDIT IS FRAUDULENT. ALL LOC-related transactions must be flagged "suspicious" or "critical" — NEVER "normal".\n\n   CRITICAL: The category "Line of Credit" can ONLY be used for transactions on the account named "Loan Adela". NEVER use "Line of Credit" for transactions on account M61750002 or any other account.\n\n   For transactions on account M61750002 (the LOC operating account):\n   - FROM M61750002 TO one of our accounts listed above → "Internal Transfer"\n   - FROM M61750002 TO an external account (not in our list) → "LOC External Transfer"\n   - NEVER categorize M61750002 transactions as "Line of Credit"\n\n   For CREDIT MEMORANDUM, ADVANCE ON LOAN, or LOAN DISBURSEMENT:\n   - Check the transaction's OWN ACCOUNT (the account field). If the transaction is ON one of our non-LOC accounts (e.g. 626433333, 741713181, etc.), then the money landed IN our account from the LOC → "LOC Funded Deposit", counterparty = "Line of Credit"\n   - If the transaction is ON M61750002 and goes to an external account → "LOC External Transfer"\n   - KEY: The account the transaction belongs to tells you WHERE the money is. If it's our account, money is here = LOC Funded Deposit.\n\n   Money going TO an LOC account → "LOC Principal Repayment" or "LOC Interest Payment"\n\n3. When you see "Transfer of Funds From X To Y", check BOTH X and Y against our accounts list.\n\n4. If counterparty is a KNOWN FRAUDULENT ACCOUNT (see below), flag as verified_fraud.\n${flaggedLines}`;
  })();
