...
export default function Home() {
  ...
  const accountContextForAI = (() => {
    ...
    return `\n## OUR ACCOUNTS (owned by us)\n${lines}\n\nLOC ACCOUNTS: ${locList}\n\nIMPORTANT RULES FOR CATEGORIZATION:\n\n1. INTERNAL TRANSFERS: A transfer is "Internal Transfer" ONLY if BOTH source AND destination are our accounts (listed above) AND neither is an LOC account.\n\n2. LINE OF CREDIT RULES (LOC accounts: ${locList}):\n   THE ENTIRE LINE OF CREDIT IS FRAUDULENT. ALL LOC-related transactions must be flagged as "suspicious" or "critical" — NEVER "normal".\n   a) Transaction ON the LOC account going TO one of our accounts:\n      → category "Internal Transfer" or "Line of Credit", direction "Internal Transfer", flag "suspicious" — this is the SENDING side, excluded from flow since the RECEIVING side captures it as LOC Funded Deposit\n   b) Transaction ON one of our accounts, description mentions FROM an LOC account:\n      → category "LOC Funded Deposit", direction "Contribution", counterparty "Line of Credit (LOC account#)", flag "suspicious" or "critical"\n      → This COUNTS as a real deposit (NOT excluded) — money is in the account\n   c) Transaction ON the LOC account going TO an EXTERNAL account (not ours):\n      → category "LOC External Transfer", direction "Withdraw", flag "critical" — real outflow, COUNTS as withdrawal\n   d) Transaction sending money TO an LOC account:\n      → category "LOC Principal Repayment" or "LOC Interest Payment", flag "suspicious"\n\n3. When you see "Transfer of Funds From X To Y", check BOTH X and Y against our accounts list.\n\n4. If the counterparty/destination is a KNOWN FRAUDULENT ACCOUNT (see below), flag as verified_fraud.\n${flaggedLines}`;
  })();
