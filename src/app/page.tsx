...
export default function Home() {
  ...
  const accountContextForAI = (() => {
    ...
    return `\n## OUR ACCOUNTS (owned by us)\n${lines}\n\nLOC-RELATED ACCOUNTS: ${locList}\n\nIMPORTANT RULES FOR CATEGORIZATION:\n\n1. INTERNAL TRANSFERS: A transfer is "Internal Transfer" ONLY if BOTH source AND destination are our accounts.\n\n2. LINE OF CREDIT RULES:\n   THE ENTIRE LINE OF CREDIT IS FRAUDULENT. ALL LOC-related transactions must be flagged "suspicious" or "critical" — NEVER "normal".\n\n   "Line of Credit" category = ONLY for transactions on "Loan Adela" account. NEVER for M61750002.\n\n3. ACCOUNT M61750002 SPECIAL RULES (JPM Brokerage/Custody):\n   This account is a PASS-THROUGH. Money enters and exits. The same amount often appears twice (positive IN, negative OUT within 1-2 days). These are NOT duplicates — they are the IN-door and OUT-door of the same money.\n\n   CREDIT MEMORANDUM (positive, loan draw arriving) → should be DISQUALIFIED. These are the funding side. The real outflow is captured by the paired FX SPOT/wire/transfer on the same or next date. Disqualify to avoid double-counting.\n\n   FX SPOT CURRENCY (negative, USD sold for foreign currency) → "LOC External Transfer", direction "Withdraw", flag "critical". This is the REAL outflow. The paired FOREIGN CASH entry (positive) is just a delivery record and should also be DISQUALIFIED.\n\n   FOREIGN CASH / MXN DELD / EUR DELD (positive Net Amount, $0 Principal) → DISQUALIFIED. These are delivery records of FX conversions. The positive amount is MISLEADING — no USD actually moved. The real cost is in the paired FX SPOT entry.\n\n   PAYMENTS with "TRANSFERRED BY WIRE" (negative) → "LOC External Transfer" if going to external party, "Internal Transfer" if going to our accounts. These are real USD outflows.\n\n   PAYMENTS with "FUNDS TRANSFERRED FROM M61750002 TO [our account]" → "Internal Transfer". Money stayed within JPM.\n\n   SECURITY PENDING / TIME DEPOSITS → "Time Deposit". Cash locked into/released from the time deposit securing the credit line.\n\n   DEBIT MEMORANDUM → "LOC Interest Payment" (interest on the credit line paid to JPM).\n\n   INTEREST ON CASH, FEES, FUND REDEMPTIONS → standalone entries, categorize normally.\n\n4. "LOC Funded Deposit" = ONLY when on our INVESTMENT accounts (NOT M61750002) and description mentions loan/advance.\n\n5. When you see "Transfer of Funds From X To Y", check BOTH X and Y against our accounts list.\n\n6. If counterparty is a KNOWN FRAUDULENT ACCOUNT (see below), flag as verified_fraud.\n${flaggedLines}`;
  })();
