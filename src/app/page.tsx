...
export default function Home() {
  ...
  const accountContextForAI = (() => {
    ...
    const loanLedgerAccounts = locAccounts.filter(a => ourAccounts.find(oa => oa.account === a && oa.notes?.toLowerCase().includes("loan")));
    return `\n## OUR ACCOUNTS (owned by us)\n${lines}\n\nLOC ACCOUNTS: ${locList}\n\nIMPORTANT RULES FOR CATEGORIZATION:\n\n1. INTERNAL TRANSFERS: A transfer is "Internal Transfer" ONLY if BOTH source AND destination are our accounts AND neither is an LOC account.\n\n2. LINE OF CREDIT RULES:\n   THE ENTIRE LINE OF CREDIT IS FRAUDULENT. ALL LOC-related transactions must be flagged "suspicious" or "critical" — NEVER "normal".\n\n   a) "Line of Credit" category = ONLY for transactions on the LOAN LEDGER account (e.g. "Loan Adela"). NOT for M61750002.\n\n   b) "LOC Funded Deposit" = ONLY when money from LOC arrives INTO one of our non-LOC accounts AND you can verify the destination is one of our accounts from the description (e.g. "Transfer From 0002 To 741713181" where 741713181 is ours). Counterparty = "Line of Credit (source#)"\n\n   c) "LOC External Transfer" = when money from LOC goes to an account NOT in our list, OR when a CREDIT MEMORANDUM does NOT clearly show money landing in one of our accounts. When in doubt → LOC External Transfer.\n\n   d) M61750002 is the LOC operating account: transactions FROM 0002 TO our accounts = "Internal Transfer". FROM 0002 TO external = "LOC External Transfer".\n\n   e) Money going TO an LOC account = "LOC Principal Repayment" or "LOC Interest Payment"\n\n3. When you see "Transfer of Funds From X To Y", check BOTH X and Y against our accounts list.\n\n4. If counterparty is a KNOWN FRAUDULENT ACCOUNT (see below), flag as verified_fraud.\n${flaggedLines}`;
  })();
